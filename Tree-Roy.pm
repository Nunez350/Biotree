...
sub write_tab_tree{
#    print "*" x 100, "\n";
    my $y_space = 2; # number of lines between two neighboring nodes
    my $root_pos = 1;
    my $maxL = 100;
    ...
    print $longest_leaf_length, "\t", $scale, "\n";
...
sub _write_tab_tree_Helper {
    ...
    for my $n ( @d ) {
	my $xc;
#	$n->{xcoord} = &distance_to_root($n);
	$xc = $n->{xcoord};
	my $branch = '-' x ($xc) ;
	    if( $n->is_Leaf ) {
        $line .= sprintf("%s|* \n|%s %s\n",  $indent,$branch,$n->id|| '');
        } else {
	    $line .= sprintf("$branch|  %s\n",( $n->id ?  sprintf("(%s)",$n->id) : ''));
        }
        my $new_indent = $indent . (($i == $lastchild) ? "-***" : " \ ");
        if( $n != $node ) {
                # avoid the unlikely case of cycles
            $line .= &_write_tab_tree_Helper($n,$new_indent);
        }
    }
#    return $line;
#*    print Dumper(\@d);
...
=begin
sub _write_tab_tree_Helper_copy {
    my ($node,$indent) = @_;
    return unless defined $node;
    my @d = $node->each_Descendent();
    my $line = "";
    ...
    for my $n ( @d ) {
	if( $n->is_Leaf ) {
	    $line .= sprintf("%s|* \n%s|-------%s\n", #line before branch start new line then branch start
                                 $indent,$indent,$n->id|| '');
#		     $line .= sprintf("%s|* \n%s|-------%s\n", #line before branch start new line then branch start
	#			 $indent,$indent,$n->id|| '');
	} else { 
